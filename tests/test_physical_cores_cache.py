"""
Tests for physical core count caching functionality.

This module tests the caching behavior of get_physical_cores() to ensure:
...
class TestPhysicalCoresCaching:
    """Test caching behavior of physical core count detection."""
    
    def test_caching_basic_behavior(self):
        """Test that physical core count is cached after first call."""
    ...
    def test_cache_persists_across_calls(self):
        ...
        cores_list = [get_physical_cores() for _ in range(10)]
    ...
    def test_cache_returns_consistent_value(self):
        ...
        # Get cores value
        expected = get_physical_cores()
        
        # Call many times and verify consistency
        for _ in range(100):
            actual = get_physical_cores()
            assert actual == expected
    
    def test_cache_reasonable_value(self):
        """Test that cached value is reasonable (1-256 cores)."""
        # Clear cache
        _clear_physical_cores_cache()
        ...
        # Should be less than 256 (reasonable upper bound for 2026)
        assert cores <= 256


class TestThreadSafety:
    """Test thread-safe operation of physical cores cache."""
    ...
    def test_concurrent_calls_safe(self):
        ...
        # Start multiple threads calling get_physical_cores concurrently
...
class TestPerformanceImprovement:
    ...
    def test_cached_calls_are_fast(self):
        ...
        # We expect at least 10x speedup from caching
        # First call may involve psutil system calls, /proc/cpuinfo parsing,
        # or even subprocess spawns (lscpu), while cached call is just a lookup
        assert time2 < time1 / 10, (
            f"Cached call ({time2:.6f}s) should be at least 10x faster than "
...
class TestIntegrationWithOptimizer:
    ...
    def test_cache_consistent_across_optimize_calls(self):
        """Test that physical cores value is consistent across optimize calls."""
        from amorsize import optimize
        ...
        # Get results from multiple optimizations
        results = [optimize(simple_func, data) for _ in range(5)]
        
        # All should succeed and get consistent system info
        # (n_jobs may vary due to memory constraints, but physical cores should be same)
